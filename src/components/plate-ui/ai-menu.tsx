...
Start writing a new paragraph AFTER <Document> ONLY ONE SENTENCE`
  ...
  discard: {
    ...
    onSelect: ({ editor }: { editor: PlateEditor }) => {
      const ai = editor.getTransforms(AIPlugin).ai;
      if (ai?.undo) ai.undo();
  ...
  [GROUP.LANGUAGES]: {
    component: TranslateMenuItems as React.ComponentType<{ menuState: EditorChatState }>,
    filterItems: true,
...
function AIMenuItems({
  ...
  const aiEditor = usePluginOption(AIChatPlugin, 'aiEditor') as PlateEditor;
  const isSelecting = useIsSelecting();
      ...
      {menuGroups.map((group, index) => (
        <MenuGroup key={index} label={group.label}>
          {group.items?.map((item: Action) => {
            const menuItem = (aiChatItems as Record<string, {
              icon?: React.ReactNode;
              label?: string;
              value?: string;
              component?: React.ComponentType<{ menuState: EditorChatState }>;
