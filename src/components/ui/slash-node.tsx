...
const groups: Group[] = [
  ...
  {
    group: 'Text color',
    items: textColorItems.map((item: { label: string; value: string }) => ({
      ...item,
  ...
  },
  {
    ...
    items: backgroundColorItems.map((item: { label: string; value: string }) => ({
      ...item,
  ...
  return (
    <PlateElement {...props} as="span">
      <InlineCombobox element={element} trigger="/">
        ...
        <InlineComboboxContent className="w-[320px]">
              ...
              {items.map(
                ({
                  ...
                  >
                    {description ? (
                      <>
                        <div className="flex size-11 items-center justify-center rounded border border-foreground/15 bg-background [&_svg]:size-5 [&_svg]:text-muted-foreground">
                          {icon}
                        </div>
