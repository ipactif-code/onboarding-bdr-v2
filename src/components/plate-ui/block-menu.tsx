...
import { turnIntoItems } from '@/components/plate-ui/turn-into-toolbar-button';
...
export const blockMenuItems = {
  ...
  caption: {
    ...
    onSelect: ({ editor }: { editor: PlateEditor }) => {
      ...
      if (firstBlock) {
        // Select the node first, then show caption
        editor.tf.select(firstBlock[1]);
        showCaption(editor, firstBlock[0]);
      }
      ...
      {menuGroups.map((group, index) => (
        <MenuGroup key={index} label={group.label}>
          {group.items?.map((item: Action) => {
            const menuItem = (blockMenuItems as Record<string, {
