...
export interface PinnedMessagesProps {
  /** The channel ID to display pinned messages for. */
...
export function PinnedMessages({
  channelId,
  className,
}: PinnedMessagesProps): React.ReactElement {
  // Real-time query for pinned messages
  const pins = useQuery(api.pins.listByChannel, { channelId }) as PinnedMessage[] | undefined;
  ...
  // Determine if user can unpin a specific pin
  // User can unpin if they are:
  // 1. A global admin (role === "admin")
  // 2. The person who created the pin
  // Note: Channel admin check happens server-side
  const canUnpin = (pin: PinnedMessage): boolean => {
    if (!currentUser) return false;

    // Global admin can unpin any message
    if (currentUser.role === "admin") return true;

    // Original pinner can unpin
    if (pin.pinnedBy === currentUser._id) return true;
  ...
  return (
    <div
      ...
      className={cn("flex h-full flex-col", className)}
    >
      <PinnedMessagesHeader pinCount={pins.length} />
...
function PinnedMessagesHeader({
  pinCount,
}: PinnedMessagesHeaderProps): React.ReactElement {
  return (
    <div
      data-slot="pinned-messages-header"
...
function PinnedMessageItem({
  ...
  canUnpin,
  onUnpin,
}: PinnedMessageItemProps): React.ReactElement {
...
function PinnedMessagesEmpty(): React.ReactElement {
  return (
    <div
      data-slot="pinned-messages-empty"
    ...
    >
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        Pin important messages to keep them visible in this channel
      </p>
    </div>
  );
}

function PinnedMessagesSkeleton(): React.ReactElement {
  return (
    <div className="space-y-2 p-4">
