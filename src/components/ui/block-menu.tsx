      ...
      {menuGroups.map((group, index) => (
        <MenuGroup key={index} label={group.label}>
          {group.items?.map((item: Action) => {
            const menuItem =
              blockMenuItems[item.value! as keyof typeof blockMenuItems];
    ...
    <>
      {menuItems.map((item) => (
        <MenuItem
          checked={value === item.value}
          icon={
            <div className="flex size-5 items-center justify-center rounded-sm border border-foreground/15 bg-background p-0.5 text-muted-foreground [&_svg]:size-3">
              {item.icon}
            </div>
          }
          key={item.value}
